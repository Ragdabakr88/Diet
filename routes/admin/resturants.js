...
//     req.checkBody('phone', messages.Phone_invlid).isMobilePhone('any');
  ...
  if (errors) {
    res.render('admin/resturants/addResturant', {
      errors,
    });
...
try{
    ...
    if(isNaN(page) || page == '0') {
        return res.redirect('/admin/resturants/?page=1');
    };
    ...
    if(page > 1 && resturants.length === 0) {
        ...
        res.render('admin/allresturants', {
            resturants,
...
router.get('/edit/:id',function (req, res) {
  ...
  res.render('admin/editresturant',{resturant:resturant});
  });
});
...
try{
  ...
  const errors = await req.validationErrors();
  if (errors) {
    res.render('admin/editresturant', {
      errors,
    });
